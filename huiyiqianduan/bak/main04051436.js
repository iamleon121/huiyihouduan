// main.js - 专门为main.html提供的JS文件，支持5+app plus模块
        ...
        if (initializePlus()) {
            ...
            if (mainView) {
                ...
                try {
                    ...
                    setTimeout(function() {
                        try {
                            mainView.evalJS('isDataFetchEnabled = true; console.log("ID定时检测已通过延迟方式恢复");');
                        } catch (retryError) {
                            console.error('重试恢复ID定时检测失败：', retryError);
                        }
                    }, 500);
                }
                mainView.show();
                ...
                newMainView.addEventListener('loaded', function() {
                    try {
                        // 在新页面加载完成后设置isDataFetchEnabled为true
                        newMainView.evalJS('isDataFetchEnabled = true; console.log("ID定时检测已在新页面中启用");');
                    } catch (error) {
                        console.error('在新页面中启用ID定时检测失败：', error);
                    }
                });
                ...
                newMainView.addEventListener('show', function() {
                    try {
                        // 确保在页面显示时也设置isDataFetchEnabled为true
                        newMainView.evalJS('isDataFetchEnabled = true; console.log("ID定时检测已在页面显示时启用");');
                    } catch (error) {
                        console.error('在页面显示时启用ID定时检测失败：', error);
                    }
                });
            }
        } else {
