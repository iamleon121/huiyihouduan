...
基于稳定性优先和快速实现的考虑，对原方案进行适当简化，去除不必要的复杂部分，保留核心功能。

### 简化原则

1. **稳定性优先**：减少动态组件，降低系统复杂度，提高系统可靠性
2. **快速实现**：选择成熟技术和简单解决方案，加快开发速度
3. **适当忽略安全性**：简化认证和授权机制，减少开发时间
4. **减少动态特性**：使用静态配置替代动态服务发现，简化系统设计
...
1. **去除动态节点注册机制**
   - 使用静态配置文件定义文件服务节点列表
   - 不再需要复杂的注册、心跳检测和健康检查流程
   - 服务器变更通过手动修改配置文件完成

2. **简化负载均衡策略**
   - 采用简单的轮询或随机选择策略
   - 去除基于健康度、存储使用率的复杂评分系统

3. **简化同步机制**
   - 使用定时任务代替事件驱动同步
   - 简化文件更新检测逻辑，使用简单的时间戳比较
   - 增加同步间隔，减少系统负载
...
# 静态配置文件服务节点
FILE_NODES = [
    {"id": "node1", "url": "http://192.168.1.101:8001"},
    {"id": "node2", "url": "http://192.168.1.102:8001"},
    {"id": "node3", "url": "http://192.168.1.103:8001"}
]
...
def select_node():
    ...
    node = FILE_NODES[node_counter % len(FILE_NODES)]
    node_counter += 1
    return node
...
async def get_meeting_data(meeting_id: str, db: Session = Depends(get_db)):
    ...
    # 构建下载URL
    if selected_node:
        download_url = f"{selected_node['url']}/api/v1/meetings/{meeting_id}/download-package"
    else:
        # 如果没有可用节点，使用主服务器URL
        download_url = f"{BASE_URL}/api/v1/meetings/{meeting_id}/download-package"
...
# 简化的同步接口
@app.get("/api/v1/sync/meetings/{meeting_id}")
async def sync_meeting_data(meeting_id: str, db: Session = Depends(get_db)):
    # 获取会议数据
    db_meeting = crud.get_meeting(db, meeting_id=meeting_id)
    if not db_meeting:
        raise HTTPException(status_code=404, detail="Meeting not found")
    ...
    # 获取会议相关文件
    meeting_files = get_meeting_files(db, meeting_id)
    for file in meeting_files:
        file_data = {
            "path": file.path,
            "size": file.size,
...
# 提供活跃会议列表API
@app.get("/api/v1/meetings/active")
async def get_active_meetings(db: Session = Depends(get_db)):
    # 获取所有进行中的会议
    active_meetings = db.query(models.Meeting).filter(models.Meeting.status == "进行中").all()
...
MAIN_SERVER_URL = os.getenv("MAIN_SERVER_URL", "http://127.0.0.1:8000")
STORAGE_PATH = os.getenv("STORAGE_PATH", "./storage")
SYNC_INTERVAL = int(os.getenv("SYNC_INTERVAL", 300))  # 同步间隔(秒) - 增加到5分钟减少同步频率
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    logger.error(f"获取进行中会议失败: {response.status}")
                    return []
    except Exception as e:
        logger.error(f"获取进行中会议出错: {str(e)}")
        return []
...
async def sync_meeting_data(meeting_id):
    try:
        ...
        # 简化的时间戳检查 - 只检查文件是否存在和是否过期
        if os.path.exists(package_path):
            # 检查文件是否超过1小时
            if time.time() - os.path.getmtime(package_path) < 3600:
                return True  # 文件存在且不超过1小时，不需要同步
        ...
        # 下载文件
        for file_info in meeting_data.get("files", []):
            file_url = file_info.get("url")
            file_name = os.path.basename(file_info.get("path", ""))
            target_path = os.path.join(files_path, file_name)
...
{
  "file_nodes": [
    {"id": "node1", "url": "http://file-node-1:8001"},
    {"id": "node2", "url": "http://file-node-2:8001"},
    {"id": "node3", "url": "http://file-node-3:8001"}
  ]
}
```
...
**优势**：
1. 快速实现 - 最短时间内完成功能
2. 简单可靠 - 减少复杂逻辑，降低错误概率
3. 易于维护 - 架构简单，代码清晰
4. 性能高效 - 减少通信开销，降低系统负载

**风险**：
1. 扩展性有限 - 节点变更需要手动配置
2. 自动化程度低 - 没有自动发现和故障转移
3. 缺乏实时监控 - 无法动态检测节点健康状态

### 缓解措施

1. 编写简单的运维脚本，辅助节点配置管理
2. 实现简单的健康检查页面，便于手动监控
3. 实现备份恢复机制，提高系统可靠性 